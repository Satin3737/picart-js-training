const scrolling = (upBtnSelector) => {
    const upBtn = document.querySelector(upBtnSelector);
    window.addEventListener('scroll', () => {
        if (document.documentElement.scrollTop > 2000) {
            upBtn.classList.replace('fadeOut', 'fadeIn');
        } else {
            upBtn.classList.replace('fadeIn', 'fadeOut');
        }
    });
    
    // scrolling with animation frame
    const links = document.querySelectorAll('[href^="#"]');
    let speed = 0.1;
    
    links.forEach(link => {
        link.addEventListener('click', function (event) {
            event.preventDefault();
            const widthTop = document.documentElement.scrollTop;
            const hash = this.hash;
            const toBlock = document.querySelector(hash).getBoundingClientRect().top;
            let start = null;
            
            requestAnimationFrame(step);
            
            function step(time) {
                if (start === null) {
                    start = time;
                }
                
                let progress = time - start;
                let r = (toBlock < 0 ? Math.max(widthTop - progress / speed, widthTop + toBlock) : Math.min(widthTop + progress / speed, widthTop + toBlock));
                
                document.documentElement.scrollTo(0, r);
                
                if (r !== widthTop + toBlock) {
                    requestAnimationFrame(step);
                } else {
                    location.hash = hash;
                }
            }
        });
    });
    
    // pure js scrolling
    // const docElement = document.documentElement;
    // const body = document.body;
    //
    // const calcScroll = () => {
    //     upBtn.addEventListener('click', function (event) {
    //         let scrollTop = Math.round(body.scrollTop || docElement.scrollTop);
    //         if (this.hash !== '') {
    //             event.preventDefault();
    //             let hashElement = docElement.querySelector(this.hash);
    //             let hashElementTop = 0;
    //
    //             while (hashElement.offsetParent) {
    //                 hashElementTop += hashElement.offsetTop;
    //                 hashElement = hashElement.offsetParent;
    //             }
    //
    //             hashElementTop = Math.round(hashElementTop);
    //             smoothScroll(scrollTop, hashElementTop, this.hash);
    //         }
    //     });
    // }
    //
    // const smoothScroll = (from, to, hash) => {
    //     let timeInterval = 1;
    //     let prevScrollTop;
    //     let speed;
    //
    //     to > from ? speed = 30 : speed = -30;
    //
    //     let move = setInterval(function () {
    //         let scrollTop = Math.round(body.scrollTop || docElement.scrollTop);
    //         if (prevScrollTop === scrollTop || (to > from && scrollTop >= to) || to < from && scrollTop <= to) {
    //             clearInterval(move);
    //             history.replaceState(history.state, document.title, location.href.replace(/#.*$/g, '') + hash);
    //         } else {
    //             body.scrollTop += speed;
    //             docElement.scrollTop += speed;
    //             prevScrollTop = scrollTop;
    //         }
    //     }, timeInterval);
    // }
    //
    // calcScroll();
}

export default scrolling;